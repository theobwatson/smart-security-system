        ...
        if GPIO.input(PIR_PIN):
            ...
            if unknown_encodings:
                ...
                if not detected:
                    ...
                    subprocess.run([
                        "telegram-send", "--image", img_path,
                        "--caption", "Motion detected: Unknown face!"
                    ])
                else:
                    print("Authorised face detected. No alert sent.")
            else:
                print("Motion detected, but no face found. Sending alert anyway.")
                subprocess.run([
                    "telegram-send", "--image", img_path,
                    "--caption", "Motion detected: No face found."
                ])
